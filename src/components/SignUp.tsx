  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-sky-950 via-emerald-950 to-amber-950 p-4">
      <div className="w-full max-w-md rounded-2xl border border-white/20 bg-white/10 backdrop-blur-xl shadow-2xl p-8">
        ...
        <p className="text-sky-100/80 mb-6">Join us and start creating amazing links.</p>
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label htmlFor="username" className="block text-sm font-medium text-white/90 mb-2">Username</label>
            <input id="username" type="text" value={username} onChange={(e)=>setUsername(e.target.value)}
              className="w-full px-4 py-3 rounded-lg border border-white/30 bg-white/10 text-white placeholder-white/60 focus:ring-2 focus:ring-sky-300/40 focus:border-transparent outline-none transition" placeholder="johndoe" required />
          </div>
          <div>
        ...
        <div className="mt-6 text-center text-sm">
          <span className="text-sky-100/80">Already have an account? </span>
          <Link to="/signin" className="text-sky-200 hover:text-white font-medium">Sign in</Link>
        </div>
      </div>
    </div>
  );
